...
LogicalResult DepthConv2dSparsityScfRewritePattern::matchAndRewrite(
    linalg::DepthwiseConv2DNchwChwOp op, PatternRewriter &rewriter) const {
  ...
  int64_t ic = resultType.getDimSize(1);
  ...
  auto IC = arith::ConstantIndexOp::create(rewriter, loc, ic); // NOLINT
  ...
  auto then = [&](OpBuilder &b, Location loc, Value Cinit, // NOLINT
    ...
    auto ow = thenArgs[3];
    auto Onchw = // NOLINT
        tensor::ExtractOp::create(b, loc, Cinit, {n, ic, oh, ow});
        ...
        [&](OpBuilder &b, Location loc, Value kh, ValueRange khArgs) {
          ...
          auto kwLoop = scf::ForOp::create(
              ...
              [&](OpBuilder &b, Location loc, Value kw, ValueRange kwArgs) {
                ...
                // For depthwise_conv_2d_nchw_chw, the output Layout is as
                // follows: O[n, ic, oh, ow] += I[n, ic, oh * SH + kh * DH, ow *
                // SW + kw * DW]
                //    * K[ic, kh, kw]
                ...
                auto Inchw = tensor::ExtractOp::create( // NOLINT
                    b, loc, input, {n, ic, ih, iw});
                auto Ffchw = tensor::ExtractOp::create( // NOLINT
                    b, loc, filter, {ic, kh, kw});
  ...
  auto nLoop = scf::ForOp::create(
      ...
      [&](OpBuilder &b, Location loc, Value n, ValueRange nArgs) {
        ...
        // -- ic loop --------------------------------------------------------
        auto icLoop = scf::ForOp::create(
            b, loc, c0, IC, c1, Cinit,
            [&](OpBuilder &b, Location loc, Value ic, ValueRange chArgs) {
              auto isFDense = checkIterDensity(b, loc, ic, fRanges);
        ...
        scf::YieldOp::create(b, loc, icLoop.getResult(0));
      });
