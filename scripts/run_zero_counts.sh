...
fill_pad_broadcast_from_output() {
  { "$GREP" -A1 -E '\[(linalg\.fill|tensor\.pad|linalg\.broadcast)\]' || true; } \
...
if [[ "$CSV_OUTPUT" == "true" ]]; then
  echo "lattice,model,seed,fill_pad_broadcast,forward,lateral,backward,total,breakoff_pct,first_matmul_pct,matmul_rw,conv_rw,depthwise_rw,poolsum_rw,poolmax_rw"
fi
...
for li in "${!LATTICE_NAMES[@]}"; do
  ...
  if [[ "$CSV_OUTPUT" != "true" ]]; then
    ...
    printf "%-25s %10s %12s %10s %10s %10s %10s | %9s %9s | %6s %6s %6s %7s %7s\n" \
      "Model" "Seed" "Fill+Pad+Bc" "Forward" "Lateral" "Backward" "Total" \
  ...
  for model in "${MODELS[@]}"; do
    ...
    fill_pad_broadcast=$(echo "$forward_out" | fill_pad_broadcast_from_output)
    ...
    if [[ "$CSV_OUTPUT" == "true" ]]; then
      printf "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" \
        "$lattice_name" "$name" "$delta_seed" "$fill_pad_broadcast" "$delta_forward" "$delta_lateral" "$delta_backward" "$after_backward" \
        "$breakoff_pct" "$matmul_pct" "$matmul_rw" "$conv_rw" "$depth_rw" "$pool_rw" "$poolmax_rw"
    else
      printf "%-25s %10s %12s %10s %10s %10s %10s | %9s %9s | %6s %6s %6s %7s %7s\n" \
        "$name" "$delta_seed" "$fill_pad_broadcast" "$delta_forward" "$delta_lateral" "$delta_backward" "$after_backward" \
