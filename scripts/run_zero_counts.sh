...
rewrite_counts_for_model() {
  local model="$1" attr="$2"
  local out matmul_rw conv_rw depth_rw pool_rw
  ...
  echo "${matmul_rw}|${conv_rw}|${depth_rw}|${pool_rw}"
}
...
for li in "${!LATTICE_NAMES[@]}"; do
  ...
  printf "%-25s %10s %12s %10s %10s %10s %10s | %9s %9s | %6s %6s %6s %7s\n" \
    "Model" "Seed" "Fill+Pad" "Forward" "Lateral" "Backward" "Total" \
    "Breakoff" "1st MM" "MM RW" "Cnv RW" "DW RW" "Pool RW"
  printf "%-25s %10s %12s %10s %10s %10s %10s | %9s %9s | %6s %6s %6s %7s\n" \
    "-----" "----" "--------" "-------" "-------" "--------" "-----" \
    "--------" "------" "------" "------" "------" "-------"
  ...
  for model in "${MODELS[@]}"; do
    ...
    IFS='|' read -r matmul_rw conv_rw depth_rw pool_rw <<< "$(rewrite_counts_for_model "$model" "$lattice_attr")"

    printf "%-25s %10s %12s %10s %10s %10s %10s | %9s %9s | %6s %6s %6s %7s\n" \
      "$name" "$delta_seed" "$fill_pad" "$delta_forward" "$delta_lateral" "$delta_backward" "$after_backward" \
      "$breakoff_str" "$matmul_str" "$matmul_rw" "$conv_rw" "$depth_rw" "$pool_rw"
  done
  echo
done
